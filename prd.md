...
**One-liner:** Instant, color-coded urgency triage for anxious pet owners in a crisis.

---

## 1. The user & the moment

- **Who:** An anxious pet owner facing a sudden, terrifying symptom or behavior in their pet. They are highly stressed, panicking, and looking for immediate, clear guidance.
- **When:** A sudden emergency (e.g., pet falls, is bleeding, choking, or struggling to breathe) often late at night when regular veterinary clinics are closed.
- **Why now:** Generic internet searches return overwhelming, conflicting, and clinical information that heightens panic. Pet owners need an instant, non-diagnostic urgency score to guide their immediate next move.

## 2. The contract (I/O)

- **Input:** A single free-text text box where the user types a description of their pet's current symptom or behavior (e.g., "My dog is choking on a bone" or "My cat has been sleeping more than usual").
...
- **The loop:** One-shot check. Open the app → type the symptom → receive the color-coded action directive → act on it.
...
- A single-screen web app with a minimalist, high-contrast, free-text symptom input field.
...
- **Primary:** ≥80% of test users report that the triage urgency category matches their expectations and provides immediate decision clarity.
